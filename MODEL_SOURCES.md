...
2. **License compliance** — every model used downstream is paired with an upstream license that is compatible with Apache-2.0.
...
- name: <short id used in CLI>
  ...
  license_compatible_with_apache2: true|false
  weight_files:
...
- **Small custom transformer** trained locally on the Groove MIDI Dataset (CC-BY-4.0). If we train and publish weights, they will be released under Apache-2.0 alongside the training script and a `DATA_SOURCES.md` reference.
...
PRs that add a model with a license that is not compatible with Apache-2.0 (e.g. non-commercial, research-only, custom EULA) will be rejected.
